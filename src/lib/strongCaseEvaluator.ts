...
export function evaluateStrongCase(input: StrongCaseInput): StrongCaseResult {
  ...
  const limiters: StrongCaseLimiters = {
    ...
    comune_only_bias: !snapshot.valore_zona_fine && (snapshot.affidabilita_valore === "Bassa" || snapshot.affidabilita_valore === "Non determinabile" || snapshot.limite_principale.includes("comunale")),
